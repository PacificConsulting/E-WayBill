page 50053 "Posted Sales Invoice Eway"
{
    ...
    begin
        ...
        /*//Below code for when Live the project at time uncomment the code
        ...
        */
        ...
        JsonEwayObject.Add('baseURL', GeneralLedgerSetup."EINV Base URL");
        JsonEwayObject.Add('access_token', GeneralLedgerSetup."Access Token");
        JsonEwayObject.Add('user_gstin', '05AAABB0639G1Z8'/*Location."GST Registration No."*/);
        JsonEwayObject.Add('supply_type', Supply);
        JsonEwayObject.Add('sub_supply_type', Subsupply);
        JsonEwayObject.Add('sub_supply_description', SubSupplydescr);
        JsonEwayObject.Add('document_type', DocumentType);
        JsonEwayObject.Add('document_number', rec."No.");
        JsonEwayObject.Add('document_date', Document_Date);
        JsonEwayObject.Add('gstin_of_consignor', '05AAABB0639G1Z8');
        JsonEwayObject.Add('legal_name_of_consignor', Location_.Name);
        JsonEwayObject.Add('address1_of_consignor', Location_.Address);
        JsonEwayObject.Add('address2_of_consignor', Location_."Address 2");
        JsonEwayObject.Add('place_of_consignor', Location_.City);
        JsonEwayObject.Add('pincode_of_consignor', Location_."Post Code");
        JsonEwayObject.Add('state_of_consignor', State_.Description);
        JsonEwayObject.Add('actual_from_state_name', State_.Description);
        JsonEwayObject.Add('gstin_of_consignee', '05AAABC0181E1ZE');
        JsonEwayObject.Add('legal_name_of_consignee', Cust.Name);
        JsonEwayObject.Add('address1_of_consignee', Cust.Address);
        JsonEwayObject.Add('address2_of_consignee', Cust."Address 2");
        JsonEwayObject.Add('place_of_consignee', Cust.City);
        JsonEwayObject.Add('pincode_of_consignee', Cust."Post Code");
        JsonEwayObject.Add('state_of_supply', StateCust.Description);
        JsonEwayObject.Add('actual_to_state_name', StateCust.Description);
        JsonEwayObject.Add('transaction_type', rec."Transaction Type");
        JsonEwayObject.Add('other_value', '');
        JsonEwayObject.Add('total_invoice_value', Format(0));
        JsonEwayObject.Add('taxable_amount', FORMAT(TotaltaxableAmtValue));
        JsonEwayObject.Add('cgst_amount', FORMAT(TotalCGSTAmt));
        JsonEwayObject.Add('sgst_amount', FORMAT(TotalSGSTAmt));
        JsonEwayObject.Add('igst_amount', FORMAT(TotalIGSTAmt));
        JsonEwayObject.Add('cess_amount', FORMAT(TotalCESSGSTAmt));
        JsonEwayObject.Add('cess_nonadvol_value', Format(0));
        JsonEwayObject.Add('transporter_id', '05AAABB0639G1Z8');
        JsonEwayObject.Add('transporter_name', rec."Transport Vendor Name");
        JsonEwayObject.Add('transporter_document_number', rec."LR/RR No.");
        JsonEwayObject.Add('transporter_document_date', Transport_Date);
        JsonEwayObject.Add('transportation_mode', rec."Transport Method");
        JsonEwayObject.Add('transportation_distance', FORMAT(rec."Distance (Km)"));
        JsonEwayObject.Add('vehicle_number', rec."Vehicle No.");
        JsonEwayObject.Add('vehicle_type', 'Regular');
        JsonEwayObject.Add('generate_status', '1');
        JsonEwayObject.Add('data_source', 'erp');
        JsonEwayObject.Add('user_ref', '');
        JsonEwayObject.Add('location_code', Location_.Code);
        JsonEwayObject.Add('eway_bill_status', FORMAT(rec."E-Way Bill Generate"));
        JsonEwayObject.Add('auto_print', 'Y');
        JsonEwayObject.Add('email', Location_."E-Mail");
        JsonEwayObjectChild.Add('ItemList', Linedata);
        // JsonEwayObjectChild.Add('product_name','');
        // JsonEwayObjectChild.Add('product_name','');
        // JsonEwayObjectChild.Add('product_name','');
        // JsonEwayObjectChild.Add('product_name','');
        Jarray.add(JsonEwayObjectChild);
        JsonEwayObject.Add('itemList', Jarray);



        JsonEwayObject.WriteTo(EwayRequestTxt);
        EwayRequestContent.WriteFrom(EwayRequestTxt);
        EwayRequestContent.GetHeaders(HttpHeadEway);


        if client.Post('https://pcazureeinvoiceintegrationuat.azurewebsites.net/api/GenerateIRN', EwayRequestContent, EwayResponse) then begin
            if EwayResponse.IsSuccessStatusCode() then begin
                ...
                Message('Eway Response Error %1', EwayResponse.HttpStatusCode);
        end else
